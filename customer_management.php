<?php    include 'list_customers.php';    include 'ban_unban_delete_customers.php';    include 'includes/db.php';    include 'includes/stats.php';?>
<!DOCTYPE html>
<html lang="en">
<head>    
  <meta charset="UTF-8">    
  ...
  <style>        
    ...
    .modal-content {            
      ...
      overflow: hidden; /* ensure rounded corners clip inner scroll */        
    }                
    .modal-header {            
      background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);            
      color: white;            
    ...
    /* Smooth pop-in animation for the popup */
    ...
    /* History modal and table enhancements */
    #historyModal .modal-body {
      background: linear-gradient(180deg, #ffffff 0%, #f7f9fc 100%);
      padding: 1.25rem;
    }
    /* Responsive, rounded, elevated wrapper for history tables */
    .history-table-wrapper {
      ...
      background: #fff;
      max-height: 60vh; /* keeps large histories scrollable inside the modal */
    }
    /* Make table header sticky for long lists */
  ...
  <script>        
    function showHistory(customerId) {            
      ...
      modal.show();                    
      document.getElementById('historyContent').innerHTML = '<div class="text-center py-5"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div><div class="mt-2 text-muted small">Fetching rental history...</div></div>';            
      const xhr = new XMLHttpRequest();            
      xhr.open('GET', 'get_rental_history.php?customer_id=' + customerId, true);            
      xhr.onload = function () {                
        const historyContent = document.getElementById('historyContent');
        if (this.status == 200) {                    
          // Enhance the returned HTML by adding Bootstrap table classes and a responsive wrapper
          ...
          if (tables.length) {
            tables.forEach((table) => {
              table.classList.add('table', 'table-hover', 'table-striped', 'align-middle', 'mb-0', 'history-table');

              // Wrap table with a responsive, styled container if not already wrapped
              if (!table.parentElement.classList.contains('history-table-wrapper')) {
                const wrapper = document.createElement('div');
                wrapper.className = 'table-responsive history-table-wrapper';
          ...
          // Inject enhanced content
          historyContent.innerHTML = '';
          // Move all children over to preserve any non-table content
          Array.from(tmp.childNodes).forEach((node) => historyContent.appendChild(node));
        } else {                    
          historyContent.innerHTML = '<div class="alert alert-danger"><i class="bi bi-exclamation-triangle me-2"></i>Error loading history.</div>';                
        }            
      };            
      xhr.send();        
    }        
...
<body>    
  ...
  <div class="container">        
    ...
    <div class="filter-container">            
      <form method="GET" action="customer_management.php" class="row align-items-center">                
        <div class="col-md-4">                    
                    ...
                    <div class="btn-group" role="group">                                            
                      ...
                      <form method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to <?= $row['status'] === 'banned' ? 'unban' : 'ban' ?> this customer?');">                                                
                        ...
                        <button type="submit" class="btn <?= $row['status'] === 'banned' ? 'btn-success' : 'btn-warning' ?> btn-action" title="<?= $row['status'] === 'banned' ? 'Unban' : 'Ban' ?> Customer">                                                    
                          <i class="bi bi-<?= $row['status'] === 'banned' ? 'person-x text-danger' : 'person-check text-success' ?>" title="<?= ucfirst($row['status']) ?>"></i>                                                
                        </button>                                            
                      </form>                                                                                                
